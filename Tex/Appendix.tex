...
\chapter{附录中的公式}\label{chap:app1}{
	...
	\begin{proof}
		...
		因为格方法只运用估计为阳性的敏感信息泄漏，所以对格方法而言，所利用的敏感信息泄漏估计的正阳性率就是所利用的敏感信息泄漏估计的准确率。敏感信息泄漏估计准确这一事件对于每个样本是互相独立的，因此对于任意一条构造签名的能量迹，恢复的泄漏都准确的概率为$p_r=p^r$。
		...
		利用签名多次构造方程并运用格方法求解实际私钥是伯努利实验。因此，如果构造方程的所有能量迹恢复的泄漏都没有假阳性，那么运用格方法一定可以求解出实际私钥。在这种情况下，多次尝试构造方程并运用格方法直至求解出私钥的方法，到恢复实际私钥为止所尝试的次数$\xi$服从参数为$p_s$的几何分布，即$\xi\sim G(p_s)$。
	...
	\begin{proposition}\label{prop:highalpha}
		...
		如果阳性样本均为真阳性，
		%如果被预测为对应的中间值$\leakedmultiplier_i=1$的电磁子迹数与所有电磁子迹数比值为$p_{pred1}$，对于被预测为对应的中间值$\leakedmultiplier_i=1$的电磁子迹敏感信息泄漏$\tilde \nonce_j$恢复准确率都为100\%，构造方程所有敏感信息泄漏没有错误时求解算法$S$一定
...
\setcounter{algorithm}{0}
	...
	\begin{breakablealgorithm}
		...
		\begin{algorithmic}[1]
			...
			\Statex \textbf{输入:} $M^\prime$:合成的迹的采样点个数
