...
\chapter{附录中的公式}\label{chap:app1}{
	...
	%因此，$\pi_{n_s}(4)$可以描述从采集的一条迹中恢复出（对于本研究使用的求解算法$S$）可利用的敏感信息泄漏的概率。
...
\chapter{附录中的图表}{
	...
	\begin{apptab}[!htb]
		\bicaption{\enspace 无噪声的敏感信息$\leakedmultiplier_i$泄漏场景下恢复的码元的信息}{\enspace Information on Scalar Symbol from Noise Free Sensitive Leakage}
		\stepcounter{app_tab}
		...
		\begin{tabular}{cc|ccc|c}
			...
			\hline
			\hline
			\multirow{4}*{1}&\multirow{4}*{3/8}&0&00&1/3&\multirow{4}*{$\hat\nonce_i=0$}\\
			\cdashline{3-5} &&1&01&2/3&\\
			\cdashline{3-5} &&2&10&0&\\
			\cdashline{3-5} &&3&11&0&\\
			\hline
			\multirow{4}*{2}&\multirow{4}*{5/16}&0&00&1/5&\multirow{4}*{$\hat\nonce_{i+129}=0$}\\
	...
	\begin{appfig}[!htb]
		...
		\end{subfigure}
		\\% line break
		\bicaption{\enspace 使用自适应数据增强方法时基于深度学习的侧信道攻击的正确密钥猜测熵}{\enspace GE of DL-SCA with adaptive DA method}
	...
	\begin{appfig}[!htb]
		...
		\end{subfigure}
		\\% line break
		\bicaption{\enspace 无数据增强时基于深度学习的侧信道攻击的正确密钥猜测熵}{\enspace GE of DL-SCA without DA}
